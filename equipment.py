import random

class NewCatch:
    def __init__(self, chances: dict, amount: int):
        self.contents: dict = {}
        self.chances: dict = chances
        for i in range(amount):
            caught = self._catch()
            self.contents[caught] = self.contents.get(caught, 0) + 1

    def _catch(self) -> str:
        catch_chance: float = random.random() * 100
        chance_sum: float = 0.0
        for fish, curr_chance in self.chances.items():
            chance_sum += curr_chance
            if catch_chance <= chance_sum:
                return fish
    
    def __str__(self) -> str:
        formatted_contents = [
            f"{fish}: {amount}"
            for fish, amount in self.contents.items()
            if fish != "nothing"
        ]
        
        if not formatted_contents:
            message = "Darn. You didn't catch anything. Womp Womp."
        elif len(formatted_contents) == 1:
            message = f"You caught a {list(self.contents.keys())[0]}"
        else:
            message = f"You caught:\n{"\n".join(formatted_contents)}"
        return message
        
class NoFishingGear:
    def __init__(self):
        self.chances: dict = {"nothing": 100.0}  # placeholder, other classes will overwrite this when inheriting
        self.capacity: int = 0

    def cast(self) -> NewCatch:
        return NewCatch(self.chances, self.capacity)

class WishDotComFishingRod(NoFishingGear):
    def __init__(self):
        self.chances: dict = {"nothing": 46.0, "cod": 20.0, "tuna": 18.0, "salmon": 16.0}  # I do not know much about fish, feel free to pull request on github, with more realistic chances
        self.capacity: int = 1